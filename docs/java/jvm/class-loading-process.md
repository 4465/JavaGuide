...
解析阶段是虚拟机将常量池(JDK7及之后常量池在堆中)内的符号引用替换为直接引用的过程。解析动作主要针对**类或接口、字段、类方法、接口方法、方法类型、方法句柄和调用限定符** 7 类符号引用进行。
...
4. 当虚拟机启动时，用户需要定义一个要执行的主类 (包含 `main` 方法的那个类, 被标明为启动类的类（`Java Test`）)，虚拟机会先初始化这个类。
