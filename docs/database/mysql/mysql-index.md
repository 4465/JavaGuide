...
​	数据散列不均匀，而且浪费存储空间，因此要求设计足够号的hash算法

**2.Hash 索引不支持顺序和范围查询(Hash 索引不支持顺序和范围查询是它最大的缺点：** 假如我们要对表中的数据进行排序或者进行范围查询，那 Hash 索引可就不行了。

3.如果存储的元素太多，会占用大量的额外存储空间而且不能狗分块存储
...
- B 树的所有节点既存放键(key) 也存放 数据(data，B树的data占用了大量的空间)，而 B+树只有叶子节点存放 key 和 data，其他内节点只存放 key。
...
>  SELECT id, name FROM table WHERE name='guang19';
